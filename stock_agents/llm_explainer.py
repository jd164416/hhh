...
class DeepSeekExplainer:
    ...
    def explain(
        ...
        prompt = self._build_prompt(ticker, trade_date, decision)
    ...
    def _build_prompt(ticker: str, trade_date: str, decision: DecisionResult) -> str:
        report_text = "\n".join(
            [f"- {r.name}: 评分 {r.score:.1f}，{r.summary}" for r in decision.reports]
        )
        return (
            f"股票: {ticker}\n"
            f"日期: {trade_date}\n"
...
def maybe_explain(
    ...
    if not api_key:
        return "未配置 DeepSeek Key，当前展示规则引擎结论。"
    try:
        return DeepSeekExplainer(api_key=api_key).explain(ticker, trade_date, decision)
    except Exception as exc:
        return f"DeepSeek 解释生成失败，已保留规则引擎结论。错误: {exc}"
